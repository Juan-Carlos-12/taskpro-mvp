            ...
            function init() {
                if (session) { showView('app-view'); loadUser(); renderTasks(); openAddPanel(); }
            ...
            function doLogin() {
                ...
                if (!user) { user = { id: Date.now(), name: email.split('@')[0], email, password: pass }; users.push(user); }
                session = { userId: user.id, name: user.name, email: user.email };
                save(); showView('app-view'); loadUser(); renderTasks(); openAddPanel();
            ...
            function doRegister() {
                ...
                if (!name||!email||!pass) return showError('Rellena todos los campos.');
                if (pass.length < 6) return showError('La contraseña debe tener al menos 6 caracteres.');
                if (users.find(u => u.email===email)) return showError('Ya existe una cuenta con ese email.');
                const user = { id: Date.now(), name, email, password: pass };
                users.push(user);
                session = { userId: user.id, name, email };
                save(); showView('app-view'); loadUser(); renderTasks(); openAddPanel();
                toast('Cuenta creada. ¡Bienvenido!');
            }

            function doLogout() { session = null; save(); showView('auth-view'); hideError(); }
            ...
            function myTasks() { return tasks.filter(t => t.userId === session.userId); }

            function addTask() {
                const title = document.getElementById('new-title').value.trim();
                if (!title) return toast('El título es obligatorio.', true);
                tasks.push({
                    id: taskIdCounter++, userId: session.userId,
                    title,
            ...
            function deleteTask(id) {
                ...
                save(); renderTasks(); toast('Tarea eliminada');
            }
            ...
            function openEdit(id) {
                ...
                document.getElementById('edit-cat').value      = t.category || 'personal';
            ...
            function saveEdit() {
                ...
                if (!t||!title) return toast('El título es obligatorio.', true);
                t.title    = title;
                ...
                save(); closeModal(); renderTasks(); toast('Tarea actualizada ✓');
            }
            ...
            const CAT_LABELS = { personal:'Personal', trabajo:'Trabajo', estudio:'Estudio', otro:'Otro' };
            const CAT_COLORS = { personal:'var(--blue)', trabajo:'var(--warn)', estudio:'var(--accent)', otro:'var(--text-dim)' };
            ...
            function renderTasks() {
                ...
                bar.style.width = pct + '%';
                bar.style.background = pct===100 ? 'var(--accent)' : pct>60 ? 'var(--blue)' : pct>30 ? 'var(--warn)' : 'var(--danger)';

                if (currentFilter === 'pendiente') list = list.filter(t => !t.done);
                if (currentFilter === 'completada') list = list.filter(t => t.done);
                if (currentFilter === 'vencida')    list = list.filter(t => isOverdue(t));

                const q = document.getElementById('search-input').value.trim().toLowerCase();
                if (q) list = list.filter(t => t.title.toLowerCase().includes(q) || (t.desc&&t.desc.toLowerCase().includes(q)));
                ...
                if (list.length === 0) {
                    ...
                    container.innerHTML = list.map(t => {
                        ...
                        return `
                        <div class="task-card ${t.done?'done':''} ${ov?'overdue':''}" id="tc-${t.id}">
                            ...
                            <div class="task-body">
                                ...
                                <div class="task-meta">
                                    ...
                                    ${t.date?`<span class="task-date ${ov?'date-overdue':''}">${ov?'⚠ ':''}${formatDate(t.date)}</span>`:''}
                                </div>
            ...
            function openAddPanel() {
                ...
                document.getElementById('add-btn-label').textContent = '✕ Cerrar';
                document.getElementById('new-title').focus();
            }
            ...
            function toggleAddPanel() {
                ...
                document.getElementById('add-btn-label').textContent = open ? '✕ Cerrar' : '+ Nueva tarea';
            ...
            function toast(msg, isError=false) {
                ...
                t.textContent = msg;
                c.appendChild(t);
                setTimeout(() => { t.style.animation='slideOut 0.3s ease forwards'; setTimeout(()=>t.remove(),300); }, 2800);
            }
            ...
            document.addEventListener('keydown', e => {
                if (e.key==='Enter') {
                    ...
                    if (lf.style.display!=='none' && document.getElementById('auth-view').classList.contains('active')) doLogin();
                }
            });
            ...
            // ════════ POMODORO ════════
            const MODES = {
                work:  { label: 'Modo trabajo',        mins: 25, color: 'var(--accent)' },
                short: { label: 'Descanso corto',      mins: 5,  color: 'var(--blue)' },
                long:  { label: 'Descanso largo',      mins: 15, color: 'var(--warn)' }
            };
                    ...
                    pomoInterval = setInterval(() => {
                        if (pomoRemaining <= 0) {
                            ...
                            if (currentMode === 'work') {
                                ...
                                showFocusTip('break');
                                if (pomoSessions >= 4) { pomoSessions = 0; updateDots(); }
                            } else {
            ...
            function updateDots() {
                document.querySelectorAll('.pomo-dot').forEach((d, i) => {
                    d.classList.toggle('filled', i < pomoSessions);
                });
            }

            // ════════ MUSIC PLAYER ════════
            const TRACKS = [
                { title: 'Lo-fi Beats',       sub: 'Lluvia & Café',        icon: '☕', freq: [220, 320, 440] },
                { title: 'Deep Focus',         sub: 'Ambiente oscuro',      icon: '🌌', freq: [110, 165, 220] },
                { title: 'Nature Sounds',      sub: 'Bosque & Agua',        icon: '🌿', freq: [330, 440, 550] },
                { title: 'White Noise',        sub: 'Sin distracciones',    icon: '📡', freq: [80, 80, 80] },
                { title: 'Piano Ambient',      sub: 'Melancolía & foco',    icon: '🎹', freq: [264, 330, 396] },
                { title: 'Binaural Beats',     sub: 'Ondas alfa 10Hz',      icon: '🧠', freq: [200, 210, 200] },
            ];
            ...
            function buildTrackList() {
                ...
                list.innerHTML = TRACKS.map((t, i) => `
                    <div class="track-item" id="track-${i}" onclick="selectTrack(${i})">
                        <span class="track-item-icon">${t.icon}</span>
                        <div class="track-item-info">
                            <div class="track-item-title">${t.title}</div>
                            <div class="track-item-sub">${t.sub}</div>
                        </div>
            ...
            function selectTrack(i) {
                ...
                document.getElementById('player-icon').textContent  = t.icon;
                playMusic();
            }
            ...
            function initAudio() {
                if (!audioCtx) {
                    ...
                    gainNode.gain.value = volValue * 0.15;
            ...
            function playMusic() {
                ...
                document.getElementById('music-viz').classList.add('active');
                updateDots();
            }
            ...
            function stopMusic() {
                ...
                musicPlaying = false;
                document.getElementById('music-play').textContent = '▶';
                document.getElementById('music-viz').classList.remove('active');
            }

            function toggleMusic() {
                if (currentTrack < 0) { toast('Selecciona una pista primero', true); return; }
            ...
            function setVolume(v) {
                volValue = parseFloat(v);
                if (gainNode) gainNode.gain.value = volValue * 0.15;
            }

            // ════════ FOCUS TIPS ════════
            const TIPS = {
                work: [
                    'Silencia el teléfono y cierra redes sociales. El foco profundo empieza con cero interrupciones.',
                    'Trabaja en UNA sola tarea. La multitarea reduce la productividad hasta un 40%.',
                    'Si algo te distrae, escríbelo y vuelve a tu tarea. Vacía la mente sin perder el hilo.',
                    'Bebe agua antes de empezar. La hidratación mejora la concentración notablemente.'
                ],
                break: [
                    'Levántate, estira el cuerpo y mira un punto lejano. Tu cerebro lo agradecerá.',
                    'Respira profundo 4 veces. Inhala 4s, sostén 4s, exhala 4s. Resetea tu mente.',
                    'Aléjate de la pantalla y da un pequeño paseo. El movimiento reactiva el cerebro.',
            ...
            function showFocusTip(type) {
                ...
                document.getElementById('focus-tip').textContent = tip;
            }
